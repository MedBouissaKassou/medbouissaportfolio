  ...
  return <div className="min-h-screen overflow-x-hidden bg-background text-foreground">
    <header className="fixed inset-x-0 top-0 z-40 border-b border-border/70 bg-background/85 backdrop-blur-xl">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-5 lg:px-8">
        ...
        <nav className="hidden items-center gap-7 md:flex" aria-label="Main navigation">
          {links.map(([to, label]) => <Link key={to} to={to} className="nav-link" activeProps={{ className: "nav-link nav-link-active" }}>{label}</Link>)}
        </nav>
      ...
      {menu && <nav className="border-t border-border bg-background px-5 py-5 md:hidden">{links.map(([to,label]) => <Link key={to} to={to} onClick={() => setMenu(false)} className="block border-b border-border py-3 text-sm font-semibold uppercase">{label}</Link>)}</nav>}
    </header>
    <main key={location.pathname} className="animate-fade-in pt-16">{children}</main>
    <footer className="border-t border-border bg-surface py-10">
      <div className="mx-auto flex max-w-7xl flex-col gap-4 px-5 text-sm text-muted-foreground sm:flex-row sm:items-center sm:justify-between lg:px-8">
        ...
        <div className="flex gap-5"><Link to="/contact" className="story-link">Start a project</Link><Link to="/auth" className="story-link">Backoffice</Link></div>
      </div>
    </footer>
